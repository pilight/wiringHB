...
cp libwiringhb.{a,so} debs/armhf/usr/local/lib/
cp wiringHB.h debs/armhf/usr/local/include/wiringhb.h

SIZE=$(du -c debs/armhf/usr/ | grep total | awk '{print $1}');

sed -e "s/Package: \(.*\)/Package: libwiringhb/g" debs/armhf/control > debs/armhf/control.new
...
DESC="GPIO interface for the Hummingboard";
DNAME="libwiringhb";
cd $DIR;
if [ $? -eq 0 ]; then
...
if [[ $1 == "purge" ]]; then
	...
	if [ -f /usr/local/lib/wiringhb* ]; then
		rm /usr/local/lib/wiringhb* 1>/dev/null 2>/dev/null;
	fi
	
	if [ -f /usr/local/include/wiringhb.h ]; then
		rm /usr/local/include/wiringhb.h 1>/dev/null 2>/dev/null;
	fi	
...
EOF
			...
			mv tmp.deb "libwirinhb-$VERSION.deb";
			mv *.deb ../../
			cd ../../
		fi